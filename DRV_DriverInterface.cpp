    ...
    )
{
    ...
    for (DWORD i = 0; i < dwNumDevices; i++)
	{
        ...
        cout << "Device : " << i << endl;
        printf("\tFlags: 0x%x %s  |  Type: %d %s  |  ID: 0x%08X  |  ftHandle: 0x%x %s",
											ptDevicesInfo[i].Flags,
                                            ptDevicesInfo[i].Flags & FT_FLAGS_SUPERSPEED ? "[USB 3]":
                                            ptDevicesInfo[i].Flags & FT_FLAGS_HISPEED ? "[USB 2]" :
                                            ptDevicesInfo[i].Flags & FT_FLAGS_OPENED ? "[OPENED]" : "",
											ptDevicesInfo[i].Type,
                                            ptDevicesInfo[i].Type == FT_DEVICE_600 || ptDevicesInfo[i].Type == FT_DEVICE_601 ? "[FT60X]": "",
											ptDevicesInfo[i].ID,
											ptDevicesInfo[i].ftHandle,
                                            ptDevicesInfo[i].ftHandle ? "[Already open]" : ""
                                            );
        cout << "\tDescription : " << wstrDescription.c_str() << endl;
        cout << "\tSerialNumber: " << wstrSerialNumber.c_str() << endl;
    }

     cout << "Debug - 1" << endl;

     a_pvOpenBy = (void *)"000000000001";
     printf("a_pvOpenBy [%s]\n", a_pvOpenBy);
    /*dwDeviceIndex = GetDeviceIndex(ptDevicesInfo, dwNumDevices, a_eTypeOpenBy, a_pvOpenBy);
	if (dwDeviceIndex == 0xFFFFFFFF)
	{
        printf("Device with %s [%s] not found!",
            a_eTypeOpenBy == EOPEN_BY_DESC ? "Description" :
            a_eTypeOpenBy == EOPEN_BY_SERIAL ? "Serial Number":
            a_eTypeOpenBy == EOPEN_BY_INDEX ? "Index" : "",
            wstrOpenBy.c_str());
    ...
    do
    {
        ...
        if (FT_FAILED(ftStatus))
        { 
            cout << __FUNCTION__  << "- FAILED! FT_GetVIDPID" << endl;
            break;
        }
        if ((uwVID != FT600_VID) || 
            (uwPID != FT600_PID && uwPID != FT601_PID) )
        {
            printf("\t%s FAILED! FT_GetVIDPID VID=0x%04x PID=0x%04x", __FUNCTION__, uwVID, uwPID);
            Cleanup();
            break;
        }
		...
		if (FT_FAILED(ftStatus))
		{
            cout << __FUNCTION__  << "- FAILED! FT_EnableGPIO" << endl;
			break;
		}
		...
		if (FT_FAILED(ftStatus))
		{
            cout << __FUNCTION__  << "- FAILED! FT_WriteGPIO" << endl;
            break;
		}
	}
    ...
    if (ftStatus != FT_OK || m_FTHandle == NULL) 
    {
        ...
        {
            cout << "Device not connected or driver not installed!" << endl;
        } 
        else 
        {
            cout << "Failed looking for device, FT_STATUS :" << ftStatus << endl;
        }
    ...
    printf("Device with %s [%s] opened! Device[%d]!",
        a_eTypeOpenBy == EOPEN_BY_DESC ? "Description" :
        a_eTypeOpenBy == EOPEN_BY_SERIAL ? "Serial Number" :
        a_eTypeOpenBy == EOPEN_BY_INDEX ? "Index" : "",
...
{
    uint32_t bufferLength = 1024*16;
    static uint32_t offset = 0;
#ifdef SIMU
      if((bufferLength - offset) >  a_ulBufferLength)
      {
        //memcpy(a_pucBuffer , dummyBuff + offset, a_ulBufferLength  );
        *a_pulLengthTransferred = a_ulBufferLength;
        offset = offset + a_ulBufferLength;
      }
      else
      {
          //memcpy(a_pucBuffer , dummyBuff + offset, (bufferLength - offset)  );
          *a_pulLengthTransferred = (bufferLength - offset);
          offset = 0;
      }
...
FT_STATUS CDriverInterface::ReadPipeEx(
    UCHAR a_ucPipeID,
    PUCHAR a_pucBuffer,
    ULONG a_ulBufferLength,
    PULONG a_pulLengthTransferred,
...
{
    return FT_ReadPipeEx(m_FTHandle,
        a_ucPipeID,
        a_pucBuffer,
        a_ulBufferLength,
        a_pulLengthTransferred,
...
BOOL CDriverInterface::IsUsb3()
{
    if (!m_bIsUsb3)
    {
        std::cout << "Warning: Device is connected using USB 2 cable and/or through a USB 2 host controller!" << std::endl;
    }
